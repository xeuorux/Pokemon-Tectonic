...
class PokeBattle_AI
	DamagingAttackDialogue                    = TrainerDialogueHandlerHash.new
	
	def self.triggerDamagingAttackDialogue(policy,battler,move,target,trainer_name,dialogue_array)
		ret = DamagingAttackDialogue.trigger(policy,battler,move,target,trainer_name,dialogue_array)
  ...
  def pbCommandPhaseLoop(isPlayer)
    ...
    loop do
      ...
      if @controlPlayer || !pbOwnedByPlayer?(idxBattler)
		...
		# Trigger events for trainer dialogue based on the chosen action
		
		# If the trainer chose a damaging move
		if @choices[idxBattler][0] == :UseMove && !@choices[idxBattler][2].statusMove?
			...
			dialogue = []
			dialogue = PokeBattle_AI.triggerDamagingAttackDialogue(:TEST,battler,move,target,trainername,dialogue)
			if dialogue.length != 0
				@scene.trainerMovesInOut(idxTrainer) {
					dialogue.each do |line|
						pbDisplayPaused(line)
					end
				}
			end
		end
...
PokeBattle_AI::DamagingAttackDialogue.add(:TEST,
