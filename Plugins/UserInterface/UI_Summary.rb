...
class PokemonSummary_Scene
	...
	def pbStartScene(party,partyindex,inbattle=false)
		...
		@sprites["itemicon"] = ItemIconSprite.new(30,320,@pokemon.firstItem,@viewport)
	...
	def drawPage(page)
		...
		@sprites["itemicon"].item = @pokemon.firstItem
		@sprites["itemicon"].type = @pokemon.itemTypeChosen
		...
		unless page == 3
			if @pokemon.hasItem?
				...
				@pokemon.items.each_with_index do |item, index|
			  	...
			  	textpos.push([itemName,16,346,0,Color.new(64,64,64),Color.new(176,176,176)])
			else
    ...
    def pbOptions
		...
		loop do
			...
			if cmdGiveItem >= 0 && command == cmdGiveItem
				...
				if item
					dorefresh = pbGiveItemToPokemon(item,@pokemon,self)
				end
