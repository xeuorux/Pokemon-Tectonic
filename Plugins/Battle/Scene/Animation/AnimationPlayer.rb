...
class RPG::Animation
    ...
    def addAnimation(otherAnim, frame, x, y) # frame is zero-based
        if frame + otherAnim.frames.length >= frames.length
            ...
            for i in frames.length...totalframes
...
class PBAnimTiming
    ...
    def to_s
        ...
        when 1
            ...
            text += format(" (opacity=%s)", @opacity.to_i)
            text += format(" (coords=%s,%s)",
               !@bgX.nil? ? @bgX : "-",
               !@bgY.nil? ? @bgY : "-")
            return text
        when 2
            ...
            text += format(" (opacity=%s)", @opacity.to_i) unless @opacity.nil?
            if !@bgX.nil? || !@bgY.nil?
                text += format(" (coords=%s,%s)",
                   !@bgX.nil? ? @bgX : "-",
                   !@bgY.nil? ? @bgY : "-")
        ...
        for i in @timing
            ...
            when 1   # Set background graphic (immediate)
                if i.name && i.name != ""
                    ...
                    bgGraphic.color   = Color.new(i.colorRed || 0, i.colorGreen || 0, i.colorBlue || 0,
i.colorAlpha || 0)
                    ...
                    bgGraphic.opacity = 0
                    bgColor.color     = Color.new(i.colorRed || 0, i.colorGreen || 0, i.colorBlue || 0,
i.colorAlpha || 0)
            ...
            when 3   # Set foreground graphic (immediate)
                if i.name && i.name != ""
                    ...
                    foGraphic.color   = Color.new(i.colorRed || 0, i.colorGreen || 0, i.colorBlue || 0,
i.colorAlpha || 0)
                    ...
                    foGraphic.opacity = 0
                    foColor.color     = Color.new(i.colorRed || 0, i.colorGreen || 0, i.colorBlue || 0,
i.colorAlpha || 0)
...
class PBAnimationPlayerX
    ...
    def initialize(animation, user, target, scene = nil, oppMove = false, inEditor = false, speedMult = 1)
        ...
        @targetOrig    = if @targetsprite
                             getSpriteCenter(@targetsprite)
                         else
                             [@viewport.rect.width / 2,
                              @viewport.rect.height - 160,]
                         end
        @oldbg         = []
        @oldfo         = []
        initializeSprites
