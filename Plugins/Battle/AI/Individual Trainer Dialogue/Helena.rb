PokeBattle_AI::TrainerSendsOutPokemonDialogue.add(:HELENA,
  proc { |policy,battler,trainer_speaking,dialogue_array|
	if battler.battle.pbAbleCount(battler.index) == battler.battle.sideSizes[1] &&
		!trainer_speaking.policyStates[:CommentedOnLastPokemonYet]
		dialogue_array.push("Don’t give me that look. I’ve won worse.")
		trainer_speaking.policyStates[:CommentedOnLastPokemonYet] = true
	end
    next dialogue_array
  }
)

PokeBattle_AI::TerrainChangeDialogue.add(:HELENA,
  proc { |policy,old_terrain,new_terrain,trainer_speaking,dialogue_array|
	if old_terrain == :Electric && !trainer_speaking.policyStates[:CommentedOnElectricLostYet]
		dialogue_array.push("...How regrettable. Sick of having your fuses blown?")
		trainer_speaking.policyStates[:CommentedOnElectricLostYet] = true
	end
    next dialogue_array
  }
)

PokeBattle_AI::TrainerIsUsingMoveDialogue.add(:HELENA,
  proc { |policy,battler,move,target,trainer_speaking,dialogue_array|
	if move.id == :TERRAINPULSE && battler.species == :CLAUNCHER &&
			!trainer_speaking.policyStates[:CommentedOnTerrainPulseYet] &&
			battler.battle.field.terrain == :Electric
		dialogue_array.push("Absolute power is at your clawtips, Clauncher. Bring down the house!")
		trainer_speaking.policyStates[:CommentedOnTerrainPulseYet] = true
	end
    next dialogue_array
  }
)