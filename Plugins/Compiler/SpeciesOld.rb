...
#      return @front_sprite_altitude > 0
    ...
    def get_family_evolutions(exclude_invalid = true)
      ...
      evos = evos.sort { |a, b| GameData::SpeciesOld.get(a[0]).id_number <=> GameData::SpeciesOld.get(b[0]).id_number }
      ret = []
      evos.each do |evo|
        ret.push([@species].concat(evo))   # [Prevo species, evo species, method, parameter]
        evo_array = GameData::SpeciesOld.get(evo[0]).get_family_evolutions(exclude_invalid)
    ...
    def get_baby_species(check_items = false, item1 = nil, item2 = nil)
      ...
      @evolutions.each do |evo|
        ...
        if check_items
          incense = GameData::SpeciesOld.get(evo[0]).incense
      ...
      ret = GameData::SpeciesOld.get(ret).get_baby_species(check_items, item1, item2) if ret != @species
      return ret
    end

    def get_related_species
      sp = self.get_baby_species
      evos = GameData::SpeciesOld.get(sp).get_family_evolutions(false)
    ...
    def family_evolutions_have_method?(check_method, check_param = nil)
      sp = self.get_baby_species
      evos = GameData::SpeciesOld.get(sp).get_family_evolutions
